...
class TileExtractor:
    ...
    def __init__(self, has_rotations=True):
        if has_rotations:
            self.horizontal_flips = [None, Image.FLIP_LEFT_RIGHT]
            self.rotation_flips = [None, Image.ROTATE_90, Image.ROTATE_180, Image.ROTATE_270]
        else:
            self.horizontal_flips = [None]
            self.rotation_flips = [None]

    def _create_flips(self, image):
        results = []
    ...
    def extract(self, reader):
        ...
        for image_bytes, image in unique_images.items():
            if image_bytes not in old_to_new_mapping:
                ...
                flips = self._create_flips(image)
